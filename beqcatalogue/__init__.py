...
# substrings (case-insensitive) found in a note that indicate a model (LLM, ML, etc), rather than
# a human tuning by ear, authored the filter; extend this list as new phrasings show up
MODEL_AUTHOR_MARKERS = [
    'ai generated',
    'ai created',
    'generated by ai',
    'created by ai',
...
def get_filter_author(note: str) -> str:
    ''' 'author' already means the curator/submitter (cf. git committer); this distinguishes who/what
    actually produced the filter (cf. git author): a human ear-tuning it, or a model '''
    if note:
        lowered = note.lower()
        if any(marker in lowered for marker in MODEL_AUTHOR_MARKERS):
            return 'model'
    return 'human'
